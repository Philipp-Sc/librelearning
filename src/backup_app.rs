...
impl AudioItem {
    ...
    fn play_audio_web(&mut self) {
        match &self.audio_element {
            ...
            None => {
                match web_sys::HtmlAudioElement::new_with_src(&self.audio_url) {
                    Ok(audio_element) => { 
    ...
    fn play_audio_native(&mut self) { 
        ...
        let mut content: Result<std::io::Cursor<bytes::Bytes>,reqwest::Error>;

        if self.audio_data.is_none() {
            let response = reqwest::blocking::get(&self.audio_url);
            match response {
                Ok(response) => {
                    match response.bytes() {
                        Ok(audio_data) => {
            
                            self.audio_data = Some(audio_data.clone());
                            content =  Ok(Cursor::new(audio_data));
                        },
                        Err(err) => {
                            content = Err(err);
                        }
            ...
            content =  Ok(Cursor::new(self.audio_data.as_ref().unwrap().clone()));
        }
        match content {
            Ok(content) => {
