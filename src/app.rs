...
impl CardDisplay {
    ...
    fn get_question(&self) -> String {                  // Translate this sentence // What is the next word? // Pronounce this sentence
        self.question_text.to_owned()
    }
    fn get_context(&self) -> String {                   // Terima kasih, selamat idul fitri. // Where are you from? // What is your name?
        self.context_text.to_owned()
    }
    fn get_label(&self) -> String {                     // Thank you, happy eid. // Kamu dari mana? // Nama kamu apa?
        self.label_text.to_owned()
    } 
    fn get_input_field_placeholder(&self) -> String {   // Type the English translation // Type the Indonesian translation // Type what you hear
...
impl LibreLearningApp { 
    ...
    fn next(&mut self) {
        ...
        for i in 0..self.card_list.len(){
            {
                let last_three_indices = (0..self.card_list[i].meta_data.scores.len()).rev().take(3);
                count_score_true += last_three_indices.clone().filter_map(|x| match self.card_list[i].meta_data.scores[x] { true => Some(true), _ => None}).count();
                count_score_false +=   last_three_indices.filter_map(|x| match self.card_list[i].meta_data.scores[x] { false => Some(false), _ => None}).count();
                count_score_undefined += if self.card_list[i].meta_data.scores.len()==0 {1usize}else{0usize};
            }
            {
                let time: f64 = estimate_next_session_timestamp(&mut self.space_repetition_model,&mut self.card_list[i].meta_data);
                let date = Date::new(&wasm_bindgen::JsValue::from_f64(time));
                super::log(&format!("Card_ID: {}\nCard_Question: {}\n{:?}\n{:?}",self.card_list[i].meta_data.id,self.card_list[i].display_data.get_context(),time,date.to_iso_string()));
            ...
            }
            {
                ...
                timeout_list.insert(self.card_list[i].meta_data.id,self.card_list[i].meta_data.timestamps.iter().filter(|&t| t >= &seven_min_ago).count() >= 3usize);
            }
        }
        self.progress = count_score_true as f32 / ((count_score_true+count_score_false+count_score_undefined) as f32);

        let reviewed_card_id = self.card_list[0].meta_data.id;  
        self.card_list.sort_unstable_by_key(|card| list.get(&card.meta_data.id)); 

        // maintaining previous sort, and sort by timeout_list
        // cards in timeout are moved to the end of the card_list.
        self.card_list.sort_by_key(|card| timeout_list.get(&card.meta_data.id).map(|x| *x as u8)); 
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.with_layout(egui::Layout::left_to_right(egui::Align::TOP), |ui| { 
                ...
                if self.show_settings {
                    ...
                    .fixed_rect(egui::Rect::from_min_size([6.0,40.0].into(),[ui.available_width()-10.0,ui.available_height()-50.0].into())) 
                    .title_bar(false)
                ...
                ui.with_layout(egui::Layout::bottom_up(egui::Align::LEFT), |ui| {
                    ui.vertical_centered_justified(|ui| {
                        if ui.add(egui::Button::new(egui::RichText::new("Connect/Edit Datasource").size(18.0).color(egui::Color32::WHITE))).clicked()  { 
     
         
                        }  
                        ui.separator(); 

                        ui.add(egui::Slider::new(&mut self.add_new_card_threshold, 0.0..=100.0).text("Add New Card Threshold"));

                        ui.checkbox(&mut self.auto_play_audio, "Auto Play Audio"); 
                        ui.checkbox(&mut self.enable_sounds, "Enable Sounds"); 
    
    
                        if ui.add(egui::Button::new(egui::RichText::new("Reset App").size(18.0).color(egui::Color32::BLACK))).clicked()  { 
                            self.reset_app = true;
                        }  
                    });});
                    });
                }
                ui.add(egui::widgets::ProgressBar::new(self.progress).desired_width(ui.available_width()-36.0).show_percentage());  

                if ui.add(egui::Button::new(egui::RichText::new("⚙").size(18.0)).fill(egui::Color32::BLACK)).clicked()  { 
            ...
            ui.heading(egui::RichText::new(self.card_list[0].display_data.get_question())); 
            ui.separator();
            ...
            ui.with_layout(egui::Layout::left_to_right(egui::Align::TOP), |ui| {
                ...
                if ui.add(egui::Button::new(egui::RichText::new(play_audio_text).size(20.0)).frame(false)).clicked()  { 

                    self.card_list[0].display_data.play_audio_item();
                    
                } 

                ui.label(egui::RichText::new(self.card_list[0].display_data.get_context()).color(egui::Color32::WHITE).size(20.0)); 
            }); 
                

            ui.allocate_space(egui::Vec2{ x: 0.0, y: 20.0,}); 
            ui.vertical_centered_justified(|ui| {
                self.card_list[0].display_data.get_image().map(|img| img.show_size(ui,egui::Vec2{ x: 200.0, y: 200.0,}));
            });
            ...
            ui.add(
                egui::TextEdit::multiline(&mut self.user_text_input)
                    .frame(false)
                    .hint_text(egui::RichText::new(self.card_list[0].display_data.get_input_field_placeholder())) // Type the Indonesian translation // Type what you hear
            ...
            ui.with_layout(egui::Layout::bottom_up(egui::Align::LEFT), |ui| {
                ...
                if self.add_new_card_threshold <= self.progress {
                    ...
                    if ui.add(egui::Button::new(egui::RichText::new("+ Add New Card").size(20.0).color(egui::Color32::BLACK)).fill(egui::Color32::GREEN)).clicked()  { 

                        self.static_audio.play_audio(&StaticSounds::MessageNewInstant);
                ...
                ui.vertical_centered_justified(|ui| {
                    ...
                    if check.clicked() {   
                        ...
                        }else{ // TODO change color for button in red blinking.
                            if self.enable_sounds {
